...
export const DeliveryTable = ({ data }) => {
  ...
  console.log('table delivery data', data)
  ...
  const transformDeliveryData = (apiData) => {
    ...
    return apiData.map((delivery, index) => {
      ...
      const deliveryId = delivery._id || delivery.delivery_id || delivery._id || delivery.id || `DEL-${index + 1}`
      const formattedId = deliveryId.toString().startsWith("#") ? deliveryId : `#${deliveryId}`

      return {
        id: formattedId,
        from: safeString(delivery.from_address || delivery.pickupLocation.address || delivery.from),
        to: safeString(delivery.to_address || delivery.deliveryLocation.address || delivery.to),
        vehicle: safeString(delivery.driving_mode || delivery.vehicle_type || delivery.vehicle),
        status: safeString(delivery.status),
        fee: safeString(delivery.delivery_fee || delivery.fee || delivery.payment.amount || "0"),
  ...
  let deliveries = []
  try {
    deliveries = transformDeliveryData(data)
  } catch (error) {
