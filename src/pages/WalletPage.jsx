...
=======
  ...
  const { getRiderByPhone, transferTo } = useWalletStore()
...
>>>>>>> Stashed changes
  ...
  const handleTransferModal = useCallback(() => {
    const handleContinue = async (userData) => {
      closeModal()

      const handleConfirm = async (amount, recipientData, transferResponse) => {
        console.log("Transfer completed:", { amount, recipientData, transferResponse })
        if  (transferResponse) {
        openModal({
          ...
          content: (
            <div>
              <p className="text-sm">
                Your transfer of <span className="text-blue-300">₦{amount}</span> to{" "}
                <span className="text-blue-300">{recipientData.name}</span> ({recipientData.phoneNumber}) was completed
                successfully.
              </p>
            </div>
          ),
